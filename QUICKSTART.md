...
> 答案被截斷 → 加大 max_tokens。真的需要更強的推理，才換更大的 model（會變慢）。
...
> 🔴 **RLS（Row Level Security）預設是關的。** 不開 = 拿到你網址的人可以讀走整張表、
> 也可以清空它。Supabase 的 anon key 本來就在前端，所以擋人的那層是 RLS，不是那把 key。
> **驗證方法**：開一個無痕視窗（沒有你的登入狀態）試著讀你的資料，讀得到就是沒生效。
